...
window.ScaleNovaAPI = (function () {
  ...
  async function submitLead(formData, options = {}) {
    ...
    const submissionId = 'SN-D04-' + new Date().toISOString().slice(0, 10).replace(/-/g, '') + '-' + Math.floor(1000 + Math.random() * 9000);
    const instantSuccessResponse = {
      success: true,
      submissionId: submissionId,
      submission_id: submissionId,
      demoId: config.demoId || 'DEMO-04',
      leadType: payload.lead_type,
      message: 'Admissions application registered. Our academic admissions office will reach out within 24 hours.'
    };
